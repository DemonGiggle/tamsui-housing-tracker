    ...
    <section class=\"tab-panel\" data-panel=\"raw\">
      ...
      <section class=\"card\">
        ...
        <ul>
          ...
          <li><strong>public-baseline</strong>：不是假成交，也不是亂編的資料；它是根據當下可取得的公開資訊（例如社區均價、格局範圍、少量可驗證樣本、社區頁基準資訊）整理出的「月度基準點」。</li>
          <li>換句話說，baseline 不是逐筆成交事實，而是目前最適合長期追蹤的月度基準層：它保留每坪價位與房型水位，讓你先看每個月的大致價格位置與變化方向。</li>
          <li>因此目前專案方向已改成：優先蒐集每月 baseline，先把社區 × 房型 × 月份的時間序列做完整；之後若拿到更強的公開樣本，再逐步覆蓋或校正 baseline。</li>
